...
MusicWindow::MusicWindow(QWidget *parent) : MainWindow(parent)
{
    ...
    lyricLabel->setMyPlayer(player);

    networkPage = new NetworkPage();
    connect(networkPage, SIGNAL(tryAddSongToCurrentList(MusicInfo)), this, SLOT(addMusic(MusicInfo)));
    connect(networkPage, &NetworkPage::lyricsAvailable, this, &MusicWindow::updateLyrics);

    networkManager = new QNetworkAccessManager(this);
    connect(networkManager, &QNetworkAccessManager::finished, this, &MusicWindow::onCoverImageDownloaded);
...
void MusicWindow::playerStateChanged(int state)
{
    switch (state)
    {
        ...
        case MyPlayer::PLAYING:
            bottomBar->setPlay();
            bottomBar->setPostionAvailable();
            lyricLabel->startLyricScroll(1000000);
            break;
    }
}

int MusicWindow::extractSongIdFromUrl(const QString& url) {
    QRegExp regex("id=(\\d+)");
    int pos = regex.indexIn(url);
    if (pos > -1) {
        return regex.cap(1).toInt(); // 返回第一个捕获组，即ID
    }
    return -1; // 如果没有找到，返回-1或其他适当的错误代码
}

void MusicWindow::musicChanged(QString listName, int index)
{
    qDebug() << "MusicWindow.musicChanged:" << endl;
    QString artist = Data::getMusicArtist(listName, index);
    ...
    qDebug() << "MusicWindow.musicChanged: music dir:" << dir << endl;
    int len = dir.length();
    ...
    dir = dir+"lrc";
    qDebug() << "MusicWindow.musicChanged: lyric dir" << dir << endl;
    lyricLabel->getFromFile(dir);
    if (dir.startsWith("http"))
    {
        int songId = extractSongIdFromUrl(dir); // 这是一个函数来提取ID
        networkPage->fetchLyrics(songId);  // 请求网络歌词
        // 获取包含 coverUrl 的 MusicInfo 对象
        MusicInfo currentSong = Data::getMusicList(listName).at(index);
        QString coverUrl = currentSong.getCoverUrl();
...
{
    QMessageBox::about(this, tr("关于Qt HTTP音乐播放器"), tr("Mail：hwh12322@outlook.com\n请勿用于任何非法途径。"));
}

void MusicWindow::updateLyrics(const QString& lyrics) {
    lyricLabel->setLyrics(lyrics);
}

void MusicWindow::downloadCoverImage(const QString &url) {
    currentCoverUrl = url; // 存储当前封面 URL
    QNetworkRequest request(url);
    request.setRawHeader("User-Agent", "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:75.0) Gecko/20100101 Firefox/75.0");

    QNetworkAccessManager* manager = new QNetworkAccessManager(this);
    connect(manager, &QNetworkAccessManager::finished, this, &MusicWindow::onCoverImageDownloaded);
    manager->get(request);
}

// MusicWindow 类中的 onCoverImageDownloaded 方法
void MusicWindow::onCoverImageDownloaded(QNetworkReply *reply) {
    if (reply->error() == QNetworkReply::NoError) {
        QByteArray imageData = reply->readAll();
        QPixmap pixmap;
        if (pixmap.loadFromData(imageData) && !pixmap.isNull()) {
            IconButton *coverButton = bottomBar->getCoverButton();
            if (coverButton) {
                bottomBar->setCoverImage(pixmap);
            }
            retryCount = 0; // 重置重试次数
        } else {
            ...
            if (retryCount < maxRetryCount) {
                retryCount++;
                QTimer::singleShot(1000, [this]() { // 重试前等待1秒
                    downloadCoverImage(currentCoverUrl); // 重新下载图片
                });
            }
        }
    } else {
        ...
        if (retryCount < maxRetryCount) {
            ...
            QTimer::singleShot(1000, [this]() {
                downloadCoverImage(currentCoverUrl);
            });
        }
    }
    reply->deleteLater();
