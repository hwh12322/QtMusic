...
QString Data::getMusicArtist(QString listName, int id)
{
    tryConnectMusicList();
    listName = listName.replace("'", "''");
    QSqlQuery query;
...
QQueue<MusicInfo> Data::getMusicList(QString listName)
{
    ...
    while (query.next())
    {
        ...
        musicInfo.setCoverUrl(query.value(5).toString());
