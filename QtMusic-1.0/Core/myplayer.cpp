...
void MyPlayer::addMusics(QString name, QQueue<MusicInfo> q)
{
    ...
    while (!q.empty())
    {
        musicInfo = q.front();
        q.pop_front();
        // 这里我们直接使用网络URL，而不是本地文件路径
        list[name].push_back(QUrl(musicInfo.getDir()));
        qDebug() << "Added music URL to player list:" << musicInfo.getDir(); // 输出添加的URL
    }
}
...
qint64 MyPlayer::getCurrentPosition() {
    qint64 position = player.position();
    //qDebug() << "Current player position:" << position;
    return position;
}
